...
/*
This is based on captZEEbo's script.
0.36 is the first version I uploaded.


CHANGELOG:
0.36:   Fixed @include URLs and other things related to https://what.cd

0.35:	Firefox 4 update

0.34:   Really fixed authkey bug this time :(

0.33:	Fixed authkey bug

0.32:   Fixed bug that broke the script.  Also added authkey reset (useful for password changes)
...
0.20:	Added links to collage when necessary.  Added better support on edit, manage, search, browse for collages.  
...
0.16:	Fixed a major bug where it did not include torrents.php*

0.15:	Added SSL support.  Got rid of annoying popup in favor of checkmark.
...
GM_registerMenuCommand("What.CD :: Collage Building Helper - Window Jump", function() {
	var windowJumpNum;
	for (var i=1;i<=3 && isNaN(windowJumpNum);i++) {
		windowJumpNum = prompt("Enter how many pixels you want the torrents.php page to auto-scroll?\n650: default, 0: none, current: " + GM_getValue("winJump", "650"));
...
if ((fullActiveURL.match(/torrents\.php/) || fullActiveURL.match(/artist\.php/)) && (collageID != 0) ) {
	var imageLink, lx;
	var matchType;
	...
	GM_xmlhttpRequest({
		...
		onload: function(response) { 
			...
			if (response.responseText.match(/Invalid authorization key/)) {
				alert('Invalid authorization key (the system must have updated your authkey since you loaded the page).  Refresh the page and try again!');//Invalid authorization key. Go back, refresh, and try again.
				lol.src = "data:image/jpeg;base64,%2F9j%2F4AAQSkZJRgABAQEASABIAAD%2F2wBDAAIBAQEBAQIBAQECAgICAgQDAgICAgUEBAMEBgUGBgYFBgYGBwkIBgcJBwYGCAsICQoKCgoKBggLDAsKDAkKCgr%2F2wBDAQICAgICAgUDAwUKBwYHCgoKCgoKCgoKCgoKCgoKCgoKCgoKCgoKCgoKCgoKCgoKCgoKCgoKCgoKCgoKCgoKCgr%2FwAARCAAQABADASIAAhEBAxEB%2F8QAFwAAAwEAAAAAAAAAAAAAAAAAAgMGB%2F%2FEACIQAAEDBAIDAQEAAAAAAAAAAAECAwQFBgcRCBITISIyUv%2FEABUBAQEAAAAAAAAAAAAAAAAAAAYI%2F8QAJBEAAQMCBAcAAAAAAAAAAAAAAQIDEQQhAAUGEhMiMkFSYXH%2F2gAMAwEAAhEDEQA%2FANvvnL%2BXLMz3Nj3S%2B9AYYWVUOEVqMCdB36JSDou%2F0T9IVrXz%2BgpGa8q3vnuiMWpJkvofmoTMpUVSjEiU%2FwAg8zjg2E9gnZC%2FSivqlPokGH5F2py6ynyKNmw7GkGlpkKRQqmhtZpzERRJDzrwHUOFI2pG%2B5UOqR10SrEFhcsMN8oYNqt2jKl0l%2Ba23Ua1HjKVT5VPKuyllw%2FKHAgEhBIX2AA3v2BderjVqQneWi5Excn5437csH3iqKfLdLp0mh0mjFeKNRCd0ICYMyQTDsSYu5xLdM4%2F%2F9k%3D";
			}
			else {
...
function update() {
var SUC_script_num = 66408; // Change this to the number given to the script by userscripts.org (check the address bar)
...
/*/testing code for rearranging page
if (fullActiveURL.match(/collage/) && !fullActiveURL.match(/manage/)) {
	var groups = document.getElementById('discog_table').getElementsByTagName('tr');
	// this line for adding line numbers
	for (var x in groups) { groups[x].innerHTML = x + groups[x].innerHTML; }
	
	var heading = new Array();
	var counter = 0;
	var i;
	for (i in groups) {
		if (groups[i].innerHTML.match(/artist.php/)) { 
			heading[counter] = i;
			counter++;
		}
		else if (groups[i].innerHTML.match(/Various Artists/)) {
			heading[counter] = i;
			counter++;
		}
	}
	...
	//for (var x = heading[3]; x < heading[4]; x++) {
	//	//groups[heading[1]].parentNode.insertBefore(groups[x], groups[heading[1]].nextSibling);
	//	//insertAfter(groups[heading[1]-1].parentNode, groups[x], groups[heading[1]-1]);
	//	groups[heading[1]].insertAfter(groups[x], groups[heading[1]]);
	//}
	var moveFrom = 5;
	var moveBefore = 4;
	moveBefore--;
	var x = heading[moveFrom] - 1;
	for (var y = x; y > (heading[moveFrom-1]-1); y-- ) {
		groups[heading[moveBefore]].parentNode.insertBefore(groups[x], groups[heading[moveBefore]]);
	}
	
	//insert after latestsitetorrents
	//latestSiteTorrents.parentNode.insertBefore(staffRecommendations, latestSiteTorrents.nextSibling);
	//alert(heading);
}*/