...
/*
CHANGELOG:
0.40   2012-10-27   Added adding torrents to the collage from torrent group and notification pages
                    Added Chromium support (with Tampermonkey)
                    Moved icon
                    Fixed script not running on uploaded/snatched pages 2 and above
                    Removed window jumping
                    Removed script update since current versions of both GreaseMonkey and Tampermonkey have built-in auto-update functions

0.36   2012-08-29   Fixed @include URLs and other things related to https://what.cd

-- anything up to this point was done by captZEEbo
...
if (fullActiveURL.match(/(torrents|artist)\.php/) && (collageID != 0))
{
    ...
    if(match != null) {
        ...
        title.insertBefore(createAddIcon('torrents.php?id='+match[2]), null);
    } else {
        ...
        for (var i =0; i<lx.length; ++i) {
       		if (lx[i].href.match(/torrents\.php\?id=(\d+)/)) {
